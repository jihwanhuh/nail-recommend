...
def apply_nail_png(base_img, mask, png_path):
    ...
    for cnt in contours:
        ...
        # -----------------------------
        # 🔥 길이(높이) 연장 비율
        # scale=1.6 → 60% 길게 붙임 (네일팁 느낌)
        # 원하면 1.2~2.0까지 조절 가능
        # -----------------------------
        scale = 1.6
        H_new = int(h * scale)

        # PNG를 (폭=w, 높이=연장된 H_new) 크기로 resize
...
def run_pipeline(pil_image, allowed_designs=None):
    ...
    if final_category is None:
        msg = "<p>❌ 마스크는 생성했으나 contour가 없습니다.</p>"
    ...
    length_kor = {"short": "짧은", "mid": "중간 길이", "long": "긴"}
    width_kor = {"narrow": "좁은", "wide": "넓은"}

    html = f"<h3>손 전체 유형: {length_kor.get(length)} · {width_kor.get(width)}</h3>"
    html += f"<p>len_wid = <b>{len_wid}</b>, 감지된 손톱 = {len(per_finger)}개</p><ul>"
    for f in per_finger:
        html += f"<li>Finger {f['finger']}: {f['category']} (aspect={f['aspect']:.2f})</li>"
    html += "</ul>"

    # TOP3 카드
    html += "<h4>TOP 3 네일팁 + 디자인 추천</h4><div style='display:flex; gap:12px;'>"
